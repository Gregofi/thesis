...
an \textit{Instruction Set Architecture} (ISA)~\cite{isa}. It is an abstract
...
personal computers often use the x86-64 while the mobile devices like
smartphones or tablets tend to use the ARM architecture~\cite{riscvscisc2}. The
ARM architecture is beginning to find its way into the computer space as well,
for example the Apple Sillicon uses ARM architecture.
...
differ. For a detailed overview of the x86-64 instruction set
see~\cite{intel-manual}.
...
.L2:
    ...
    ret
    \end{lstlisting}
    \caption{Compiled program from the C programming language to the x86-64
    assembly with the GCC compiler.}
...
at once and can execute them in non sequential order. The observable behaviour
...
execution of an instruction consist of several stages, which together form an
...
depending on architecture, but the general ideas behind it are the same. The
instruction must pass all these stages to be successfully executed. To maximize
efficiency, all stages are occupied by an instruction. This means that when
instruction $x$ leaves fetch stage, it goes to the decode stage and instruction
$y$ is put into the fetch stage.
...
However, putting a breakpoint on the instruction that will be executed next
...
void step_over_bp(pid_t pid) {
    ...
    \end{minted}
    \caption{Stepping over an active breakpoint.}
    \label{fig:singlestep}
\end{figure}

For a step in, we can just use the \texttt{PTRACE\_SINGLESTEP}
...
\ref{section:cpu-debug-support} we mentioned that CPU also supports
...
Linux operating system.\todo{Kratky, asi nema cenu aby to byla subsection}
...
examples\todo{Should I cite all of these guys?}. The information DWARF encodes

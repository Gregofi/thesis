...
\mintinline{c}{OutputDebugString}, and so on. One category of events are
exceptions, let us look at them more closely.
...
that requires the execution of code outside the normal flow of control. There
are software exceptions, like throwing an exception explicitly or by OS, and
...
thrown, if the execution can continue after handling the exception, the address
where the exception occurred and some others\footnote{See MSDN documentation
\cite{windows-msdn-seh} for full detailed list}. The system then searches for
an exception handler that will handle the exception. The search is performed in
this order:
...
opportunity, called \textit{last-chance}, is when a debugee doesn't have an
appropriate frame-based handler to handle the exception. If the debugger
weren't in the way, the system would have already used the default exception
...
\todo{ref}. For illustration, figure \ref{fig:windows-debugger-mainloop}
contains an abbreviated main loop of the debugger. We not only need to handle
...
\ref{fig:ast}). It also checks that the source code complies with the grammar
of the language. The semantic analysis then checks that the program is
semantically consistent. For example that used variable has been declared
before.
...
easily translated, but retains some high-level properties that make it easier
...
Not all languages are translated into machine code. Sometimes an interpreter is
...
both a compiler and an interpreter, like Haskell GHC. Since the interpreter is
a program, it is another layer of abstraction. This can make the resulting
language more abstract than the compiled ones, sometimes at the cost of
performance~\cite{jit}. 
...
the instruction at address $0xABCD$ belongs to line $x$ in the source code. Or
we need to provide information about where is some variable located. Before we
get into that, we will take a look at the layout of executables and where are
those pieces of information stored.

\subsection{ELF}
ELF means Executable and Linkable format, it is a format used for binary
applications. The file itself consists of sections, some of which are
\begin{itemize}
...
section, the size of the section, etc. The ELF format are mainly used for
\textit{relocatable} file, \textit{executable} file and \textit{shared object}
file \cite{elf}. We will talk only about executables since that is most
relevant to us. The sections themselves are often protected. For example, the
.text section can only be read and executed, but cannot be written into. The
.data section can be read and written, but cannot be executed.
...
different information that may be needed while debugging and to encode them in
...
stored in various debug headers if used in conjunction with the ELF standard.
...
bytecode language that one has to interpret to gain this information. The
bytecode program is then encoded in the executable and one can build the matrix
from this bytecode program.
...
pieces of information. Then \textit{Standard opcodes} are provided, which are
...
It also has \textit{Special opcodes}, which affect the virtual machine in the
following way:
\begin{itemize}
...
one byte this technique is very efficient on space, which is why DWARF uses it.
...
DIE) and can own multiple DIEs. There are also other relations among the DIEs,
not only ownership. With those relations in place, the relation is a graph, not
just a tree.
...
DIE. That DIE only contains information that its a const, and has an attribute
that again contains the address of another DIE. The die at \texttt{0x6b}
finally has a base type. It contains the size of the type, name, and encoding.
...
which line and column it is, at which address the function begins and ends, and
so on. We can see that this DIE has children (by indentation). Here, the
children are the parameters and variables. Important information about
variables is where they are stored, so that we may look up their value.
...
Contrary to the virtual machine in-line encoding, which was register-based,
this one is stack-based. This means that most operations take their operands
from the stack and push the result back onto the stack. More about stack-based
virtual machines can be discovered in \cite{crafting-interpreters}.DWARF calls
those instructions \textit{expressions} \cite{dwarf}. The last value on the top
of the stack after the VM runs is considered to be the value of the variable
...
the computed value on the top of the stack is the resulting value of the
variable that the debugger will get.
...
many more instructions, all of which can be found in \cite{dwarf}. The
...
seen in figure \ref{fig:stack-layout}. In that example there is the stack frame
of the previous function and the new stack frame, which mainly consist of the
following:
\begin{itemize}
...
know exactly where was the base pointer saved so that we may simulate the
unwinds.
...
stack frame) is at the provided offset. The offset is to some register that is
...
happens when the instruction \texttt{mov \%rsp, \%rbp} is done, the stack frame
is now offset by 16 from the base pointer. This holds until the epilogue, where
...
square/.style = {draw=black!60, fill=white!5, very thick, 
        ...
        \node (pro1) [square] {Old stack frame};
        \node (pro2) [square,
                      right=of pro1] {Return address};
        \node (pro3) [square,
                      right=of pro2,
                      text width=2cm] {Base pointer backup}; 
        \node (pro4) [square, right=of pro3,
                      text width=3cm] {Potential backup of other registers}; 
        \node (pro5) [square, right=of pro4,
                       text width=2cm] {Allocated space for variables}; 
    \end{tikzpicture}

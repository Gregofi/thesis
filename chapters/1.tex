...
general. The Linux and other POSIX compliant operating system use signals,
...
caused by interrupts. SEH unifies both of these things into one just like
signals do.
...
Now that we have a tool through which breakpoints and singlestepping may be implemented,
lets look at other tools that the Windows API gives us. On Linux, we had a single function
called \texttt{ptrace} which behavior changed based on arguments. Windows on the other hand
provides us with many functions, some of which are:

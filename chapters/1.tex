...
seen on figure \ref{fig:stack-layout}. In that example there is the stack frame
...
The figure \ref{fig:dwarf-stack-frames} contains an example of the encoding of
...
\begin{itemize}
    ...
    \item Array types - A sequential storage space containing objects of the
        ...
        Interesting attribute that multidimensional array has is ordering -
        either column or row wise. This is not relevant to C, since it has no
        notion of multidimensional array.
...
debugger for C, Objective-C and C++ (LLVM tools supports mainly those three
languages) on the desktop and iOS devices. It works closely with the Clang
...
\todo{Check if visual studio can do the same, if ye then throw this guy away.}

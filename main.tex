...
ladicí programy. Aby debuggery fungovaly, musí překladač emitovat ladicí
informace popisující vazby mezi generovaným kódem a zdrojovým kódem. Virtuální
stroj nepodporuje téměř žádné ladění. Studenti tedy nemají žádný důvod
generovat ladicí informace.

Tato práce popisuje, jak je podpora ladění poskytována na různých úrovních,
jako je procesor a operační systém. Popisuje také, jak vytvořit velmi
jednoduchý ladicí program v systému Linux. Práce dále popisuje přidávání
podporu ladění do samotného virtuálního stroje Tiny x86, přičemž se inspirujeme
skutečnými případy. Nakonec je vytvořen debugger na úrovni assembleru a
zdrojového kódu opět inspirovaný návrhem debuggerů z reálného světa.
}
\abstractEN{
...
the connections between generated code and the source code. The virtual machine
supports almost no debugging. Hence the students do not have any reason to
...
then add debugging support to the Tiny x86 virtual machine itself, inspired by
real-world cases. Finally, we create an assembly and source-level debugger
inspired by real-world debuggers' design.
}
